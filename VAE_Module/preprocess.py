...
def generate_data(seq, return_data=False):
    ...
    np.save('/efs/workspace/traj_pred_vae/train_data/' + str(y_time_steps) + '/X_' + '{0:0=2d}'.format(int(seq)), np.array(X).astype(np.float32))
    np.save('/efs/workspace/traj_pred_vae/train_data/' + str(y_time_steps) + '/y_' + '{0:0=2d}'.format(int(seq)), np.array(y).astype(np.float32))
